...
- **No changes to Graphify's own ranking logic** (exact/product-scoped/semantic paths, weighting, boosts) — this feature works entirely through the existing "local KB is checked before Graphify, and wins on a good match" behavior in `kb_search()`.
